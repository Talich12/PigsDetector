...
def main():
    # Запускаем mediamtx в отдельном потоке
    mtx_thread = Thread(target=run_mediamtx)
    mtx_thread.daemon = True
    mtx_thread.start()

    # Даем mediamtx время на запуск
    import time
    time.sleep(2)
